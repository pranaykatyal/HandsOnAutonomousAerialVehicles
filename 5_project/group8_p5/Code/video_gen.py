...
def create_detailed_flow_video(overlay_dir='./run', frames_dir='./imgs', output_dir='./videos', fps=10):
    """
    Generate a video that shows intermediate frames followed by detailed flow visualizations.
    For each iteration j, shows:
    1. All _iter_j_frame_n.png frames
    2. flow_image_j.png for 0.5 seconds
    3. masked_flow_viz_j.png for 0.5 seconds
    4. largest_mask_resized_j.png for 0.5 seconds
    5. overlay_j.png for 0.5 seconds
    
    Parameters:
    - overlay_dir: Directory containing overlay images and flow visualizations
    - frames_dir: Directory containing _iter_j_frame_n.png images
    - output_dir: Directory to save output video
    - fps: Frames per second for video playback
    """
    ...
    # Create temporary directory for sequenced frames
    temp_dir = '/tmp/detailed_flow_frames'
    if os.path.exists(temp_dir):
        shutil.rmtree(temp_dir)
    os.makedirs(temp_dir)
    ...
    # Build sequence of frames
    frame_counter = 0
    viz_duration_frames = int(0.5 * fps)  # 0.5 seconds per visualization
    ...
    for idx, overlay_file in enumerate(overlay_files):
        ...
        # Define the visualization files to show in sequence
        viz_files = [
            (os.path.join(overlay_dir, f'flow_image_{iter_num}.png'), f'flow_image_{iter_num}'),
            (os.path.join(overlay_dir, f'masked_flow_viz_{iter_num}.png'), f'masked_flow_viz_{iter_num}'),
            (os.path.join(overlay_dir, f'largest_mask_viz_{iter_num}.png'), f'largest_mask_viz_{iter_num}'),
            (overlay_file, f'overlay_{iter_num}')
        ]
        
        # Add each visualization for 0.5 seconds
        for viz_path, viz_name in viz_files:
            if os.path.exists(viz_path):
                ...
                if viz_img is not None:
                    ...
                    if frame_counter > 0:
                        first_frame = cv2.imread(os.path.join(temp_dir, 'frame_000000.png'))
                        if first_frame is not None and viz_img.shape != first_frame.shape:
                            viz_img = cv2.resize(viz_img, (first_frame.shape[1], first_frame.shape[0]))
                    else:
                        # Ensure dimensions are even
                        h, w = viz_img.shape[:2]
                        if h % 2 != 0 or w % 2 != 0:
                            new_h = h if h % 2 == 0 else h + 1
                            new_w = w if w % 2 == 0 else w + 1
                            viz_img = cv2.resize(viz_img, (new_w, new_h))
                    
                    print(f"Adding {viz_name} for {viz_duration_frames} frames (0.5 seconds)")
                    
                    for _ in range(viz_duration_frames):
                        output_frame = os.path.join(temp_dir, f'frame_{frame_counter:06d}.png')
                        cv2.imwrite(output_frame, viz_img)
                        frame_counter += 1
            else:
                print(f"Warning: {viz_name}.png not found")
    ...
    # Use ffmpeg to create video with proper encoding
    output_path = os.path.join(output_dir, 'detailed_flow_animation.mp4')
    ...
    try:
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        print(f"Successfully created detailed flow animation video: {output_path}")
...
def create_video_from_folder(folder_path, output_file, fps=10, pattern='*.png'):
    """
    Create a video from numbered image frames in a folder using ffmpeg.
    
    Parameters:
    - folder_path: Path to folder containing numbered image frames
    - output_file: Output video filename (will add .mp4 if not present)
    ...
    Example:
        create_video_from_folder('./my_frames', 'output_video.mp4', fps=30)
        create_video_from_folder('./images', 'result', fps=15, pattern='*.jpg')
    """
    import cv2
    ...
    image_pattern = os.path.join(folder_path, pattern)
    image_files = glob.glob(image_pattern)
    
    if not image_files:
        print(f"Error: No images found matching pattern '{pattern}' in {folder_path}")
        return False
    
    # Sort images naturally (handles numbered filenames correctly)
    image_files = sorted(image_files, key=natural_sort_key)
    
    print(f"Found {len(image_files)} images in {folder_path}")
    print(f"First frame: {os.path.basename(image_files[0])}")
    print(f"Last frame: {os.path.basename(image_files[-1])}")
    
    # Check first image dimensions and ensure they're even for libx264
    ...
    # Check if dimensions need adjustment (must be even for libx264)
    needs_resize = (h % 2 != 0) or (w % 2 != 0)
    
    if needs_resize:
        # Create temporary directory for resized frames
        temp_dir = '/tmp/video_frames_resized'
        if os.path.exists(temp_dir):
            shutil.rmtree(temp_dir)
        os.makedirs(temp_dir)
        ...
        new_h = h if h % 2 == 0 else h + 1
        new_w = w if w % 2 == 0 else w + 1
        print(f"Resizing images to {new_w}x{new_h} (dimensions must be even for h264 encoding)")
        ...
        for i, img_file in enumerate(image_files):
            ...
            if img is not None:
                ...
                temp_file = os.path.join(temp_dir, f'frame_{i:06d}.png')
                cv2.imwrite(temp_file, resized)
                resized_files.append(temp_file)
        
        # Use resized images
        result = ffmpeging_video(resized_files, output_file, fps=fps)
        ...
        return ffmpeging_video(image_files, output_file, fps=fps)
