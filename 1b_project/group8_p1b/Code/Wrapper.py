    ...
    try:
        ...
        def generateSigmaPoints(x_mean, P, Q):
            ...
            for noise_6d in sigma_points_6d:
                rotation_noise = noise_6d[0:3]
                w_noise = noise_6d[3:6]        
            ...
            return np.array(sigma_points_7d) 
        ...
        def ukfPredict(sigma_points, dt):
            ...
            for i in range(12):
                ...
                q_current = x_current[0:4]  
                w_current = x_current[4:7] 

                w_new = w_current  
                
                alpha = np.linalg.norm(w_current) * dt      
                
                if alpha < 1e-8:  
                    q_delta = np.array([1, 0, 0, 0])
                else:
                    axis = w_current / np.linalg.norm(w_current) 
        ...
        def getOrientationFromUKF(Physw, PhysAcc, ValidMask, dtar, initial_orientation):
            ...
            # print(f"UKF state angular velocity: {x_current[4:7]}")
            ...
            for i, dt in enumerate(dtar):
                ...
                quat = x_current[0:4] / np.linalg.norm(x_current[0:4]) 
        ...
        if not is_test_data:
            ...
            initial_orientation[1] = -vicon_raw[1]  
